...
teacherRouter.post("/add", async (req, res) => {
  if (!validator.isJSON(req.body.args)) {
    throw new Error("validation of body parameter failed");
  }
...
studentRouter.use((_req, res, _next) => {
  res.status(404).send("Sorry can't find that!")
})

studentRouter.use((err, _req, res, _next) => {
  console.error(err.stack)
  res.status(500).send('Something broke!')
})

teacherRouter.use((_req, res, _next) => {
  res.status(404).send("Sorry can't find that!");
});

teacherRouter.use((err, _req, res, _next) => {
  console.error(err.stack);
  res.status(500).send("Something broke!");
});
